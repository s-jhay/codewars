...
public class Kata
{
  ...
  {
    Dictionary<int, int> seenIndices = new();
    
    for (int i = 0; i < numbers.Length; i++)
    {
      int complement = target - numbers[i]; 

      if(seenIndices.TryGetValue(complement, out int complementIndex))
      {
        return new [] {complementIndex, i};
      }

      seenIndices.TryAdd(numbers[i], i);
    }
    
    throw new ArgumentException("Given numbers can't add up to target.");
  }
}
